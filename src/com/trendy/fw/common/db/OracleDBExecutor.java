...
public class OracleDBExecutor extends DBExecutor {
	...
	public <E> List<E> getRecordListResult(String sql, List<String> paramList, Class<E> clazz, PageBean pageBean,
		...
		ps.setInt(index + 1, pageBean.getEndRecordNum());
		ps.setInt(index + 2, pageBean.getStartRecordNum());
	...
	public String parsePageSql(String sql, int startNum, int endNum) {
		return "select * from (select PAGE.*, rownum rn from (" + sql + ") PAGE where rownum <= " + endNum
				+ ") where rn > " + startNum;
	}
}
